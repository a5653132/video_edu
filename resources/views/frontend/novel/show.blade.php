...
@section('content')
                ...
                <div class="card">
                    <div class="card-body">
                        <ul class="nav nav-tabs">
                            <li class="nav-item">
                                <a class="nav-link active" data-toggle="tab" href="#intro">文章介绍</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-toggle="tab" href="#videos">文章列表</a>
                            </li>
                        </ul>
                                        ...
                                        @foreach($course->getChaptersCache() as $chapter)
                                            ...
                                            <div class="media-list-body">
                                                @foreach($chapter->getVideosCache() as $video)
                                                    ...
                                                    <a class="media media-single" href="{{route('novel_content.show', [$video->novel_id, $video->id, $video->slug])}}">
                                                        <h5 class="title">
                                                            {{$video->title}}
                                                            @if($video->charge > 0)<br><span class="badge badge-primary">收费</span></br>@endif
                                                        </h5>
                                                        <time datetime="{{$video->published_at}}"></time>
                                                    </a>
                                                @endforeach
                                            </div>
                                        @endforeach
    ...
    @include('components.frontend.comment', ['submitUrl' => route('ajax.novel.comment', $course), 'comments' => $comments])
