...
class User extends Authenticatable
{
     ...
     * @return \Illuminate\Database\Eloquent\Relations\BelongsToMany
     */
    public function joinNovels()
    {
        return $this->belongsToMany(Novel::class, 'user_novel', 'user_id', 'novel_id')
            ->withPivot('created_at', 'charge');
    }
     ...
     * @return \Illuminate\Database\Eloquent\Relations\BelongsToMany
     */
    public function buyNovelContents()
    {
        return $this->belongsToMany(NovelContent::class, 'user_novel', 'user_id', 'novel_id')
            ->withPivot('created_at', 'charge');
    }
     ...
     * @param Course $course
     */
    public function joinANovel(Novel $course)
    {
        if (! $this->joinNovels()->whereId($course->id)->exists()) {
            $this->joinNovels()->attach($course->id, [
                'created_at' => Carbon::now()->format('Y-m-d H:i:s'),
                'charge' => $course->charge,
            ]);
        }
    ...
    public function canSeeThisNovelContent(NovelContent $novalContent)
    {

        $course = $novalContent->novel;

        if ($course->charge == 0 || $novalContent->charge == 0) {
            return true;
        }
        ...
        // 是否加入课程
        $hasJoinCourse = $this->joinNovels()->whereId($novalContent->novel->id)->exists();
        if ($hasJoinCourse) {
            return true;
        }

        // 是否购买视频
        return $this->buyVideos()->whereId($novalContent->id)->exists();
