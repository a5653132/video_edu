...
class SearchController extends Controller
{
    ...
    public function novelSearchHandler(SearchRequest $request)
    {
        $keywords = $request->input('keywords');
        $videos = collect([]);

        if ($keywords) {
            $videos = NovelContent::where('title', 'like', "%{$keywords}%")
                ->published()
                ->show()
                ->orderByDesc('published_at')
                ->limit(20)
        ...
        return v('frontend.novel_search.index', compact('videos'));
    }
}
